...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                assert self.ant.is_container != insect.is_container, 'Two ants in {0}'.format(self)
                if insect.is_container:
                    insect.contain_ant(self.ant)
                    self.ant = insect
                else:
                    assert self.ant.contained_ant is None, 'Two ants in {0}'.format(self)
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect, QueenAnt) and not insect.is_impostor:
                return
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        cur = self.place
        while (cur.bees == []) and (cur.entrance != hive):
            cur = cur.entrance
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            bees_cp = self.place.bees[:]
            for bee in bees_cp:
                bee.armor -= self.damage
            bees_cp = [bee for bee in bees_cp if bee.armor > 0]
            self.place.bees = bees_cp
            self.place.remove_insect(self)
        # END Problem 5
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        self.armor = 1
        if not QueenAnt.has_been_put:
            self.is_impostor = False
            QueenAnt.has_been_put = True
        else:
            ...
            while cur is not None:
                if cur.ant:
                    ...
                    if cur.ant.is_container:
                        ants += [cur.ant.contained_ant]
                cur = cur.exit
            for ant in ants:
                if (ant is not None) and (not hasattr(ant, 'has_been_encouraged')):
                    ant.damage *= 2
                    ant.has_been_encouraged = True
        # END Problem 13
