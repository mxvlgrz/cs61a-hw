...
class InstructorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for deck_card in player.deck.cards:
            deck_card.attack += 300
            deck_card.defense += 300
        player.deck.cards += [other_card.copy()]
        player.hand += [other_card.copy()]
        opponent.deck.cards += [other_card.copy()]
        opponent.hand += [other_card.copy()]
        #Uncomment the line below when you've finished implementing this method!
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for player_deck_card in player.deck.cards:
            player_deck_card.attack += atk
            player_deck_card.defense += dfs
        opponent.deck.cards = [card for card in opponent.deck.cards if not card.attack == atk and card.defense == dfs]
