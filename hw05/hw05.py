...
class MissManners:
    ...
    def ask(self, message, *args):
        ...
        "*** YOUR CODE HERE ***"
        order = message[7:]
        if not hasattr(self.obj, order):
            return 'Thanks for asking, but I know not how to %s.' % order
        func = getattr(self.obj, order)
        return func(*args)
...
def long_paths(t, n):
    ...
    "*** YOUR CODE HERE ***"
    rst = []
    search_long_paths(t, n, [], rst)
    return rst

def search_long_paths(t, n, path, rst):
    cur_path = path + [t.label]
    if t.branches == [] and len(cur_path) > n:
        rst += [cur_path]
    for b in t.branches:
        search_long_paths(b, n, cur_path, rst)
